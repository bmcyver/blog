...
`EJS`에서 `RCE`가 발생하는 이유는 간단하다. `EJS`는 `JavaScript`를 사용하는 템플릿이기 때문이다.

먼저, 왜 `EJS`에서 `RCE`가 발생하는지 보다, 어떻게 `EJS`가 작동하는지를 알아보자.

먼저, `express`에서 `render`를 호출하면 `renderFile`이 호출된다.
...
```javascript title="lib/ejs.js (v3.1.10)" showLineNumbers startLineNumber=589
...
```javascript title="lib/ejs.js (v3.1.10)" showLineNumbers startLineNumber=654
try {
	if (opts.async) {
...
```javascript title="lib/ejs.js (diff v3.1.6 <-> v3.1.7)" showLineNumbers startLineNumber=589 ins={2-4,7-9,14-16}
...
```javascript title="lib/ejs.js (v3.1.7)" showLineNumbers startLineNumber=636
if (opts.client) {
...
```javascript title="lib/ejs.js (diff v3.1.7 <-> v3.1.10)" showLineNumbers startLineNumber=509 ins={3-4} del={1-2}
